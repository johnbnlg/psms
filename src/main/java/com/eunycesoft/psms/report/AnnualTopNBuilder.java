...
public class AnnualTopNBuilder extends BaseReportBuilder {

    private int number;

    private Period period;

    protected AnnualTopNBuilder(Period period, Integer number, String lang) {
        ...
        dataSource = new JRBeanCollectionDataSource(repo.findByLanguage(Language.valueOf(lang.toUpperCase()))
                .stream().filter(rs->rs.getTerm2Average() > 0).toList());
    ...
    public BaseReportBuilder buildReport() {
        ...
        columnsBuilders = List.of(
                ...
                avg = col.column("term2Average", Double.class).setPattern(averagePattern),
